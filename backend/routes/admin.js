...
// ── ABOUT PAGE (single editable story text) ───────────────────────────────────
router.get('/story', adminOnly, (req, res) => {
  const db = load();
  res.json(db.story || { title: 'Our Story', content: '' });
});

router.put('/story', adminOnly, (req, res) => {
  const { title, content } = req.body;
  const db = load();
  db.story = { title: title || 'Our Story', content: content || '' };
  save(db);
  res.json(db.story);
});

// ── STORIES (multiple timeline entries for About page) ────────────────────────
...
router.put('/stories/:id', adminOnly, (req, res) => {
  const db = load();
  if (!db.stories) return res.status(404).json({ error: 'Story not found' });
